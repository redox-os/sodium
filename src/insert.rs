    ...
    pub fn delta(&'a self) -> usize {
        ...
        match self.cursor().mode {
            ...
            Mode::Primitive(PrimitiveMode::Insert(InsertOptions { mode: InsertMode::Append }))
                if x == self.buffer.get_line(y).len() => 0,
    ...
    pub fn insert(&'a mut self, k: Key, InsertOptions { mode }: InsertOptions) {
        ...
        match mode {
            InsertMode::Insert | InsertMode::Append => {
                ...
                match k {
                    Key::Char('\n') => {
                        let begin = self.buffer.insert_newline(x + d, y, self.options.autoindent);
                    ...
                    Key::Backspace => {
                        // NEEDS REWRITE {
                        // Backspace
                        let del = if self.buffer.get_line(y).len() == 0 {
                            1
                        } else if d == 0 && x == 0 {
                    ...
                    Key::Char(c) => {
                        self.buffer.get_line(y).insert(x + d, c);

                        // TODO: Are there a better way than switching?
                        match mode {
                            InsertMode::Insert if x + 1 == self.buffer.get_line(y).len() => {
                                self.cursor_mut().mode =
                                    Mode::Primitive(PrimitiveMode::Insert(InsertOptions {
                                        mode: InsertMode::Append,
                                    }));
